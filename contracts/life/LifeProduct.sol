...
import "./LifeOracle.sol";

contract LifeProduct is Product {
    // constants
    ...
    // events
    event LogLifePolicyCreated(
        bytes32 processId,
        address policyHolder,
        uint256 sumInsured,
        string objectName
    );
    event LogLifePolicyExpired(string objectName, bytes32 processId);
    event LogLifeOracleCallbackReceived(
        uint256 requestId,
        bytes32 processId,
        bytes LifeCategory
    );
    event LogLifeClaimConfirmed(
        bytes32 processId,
        uint256 claimId,
        uint256 payoutAmount
    );
    event LogLifePayoutExecuted(
        bytes32 processId,
        uint256 claimId,
        uint256 payoutId,
        uint256 payoutAmount
    ...
    function applyForPolicy(
        ...
        // trigger Life observation for object id via oracle call
        requestId = _request(
            processId,
    ...
    function expirePolicy(bytes32 processId) external onlyOwner {
        ...
        emit LogLifePolicyExpired(objectName, processId);
    }
    ...
    function oracleCallback(
        ...
        emit LogLifeOracleCallbackReceived(requestId, policyId, response);
...
*/
        ...
        bytes1 LifeCategory = abi.decode(response, (bytes1));

        // Claim handling based on reponse to greeting provided by oracle
        _handleClaim(policyId, policyHolder, sumInsured, LifeCategory);
    }
    ...
    function _handleClaim(
        ...
        uint256 payoutAmount = _calculatePayoutAmount(sumInsured, LifeCategory);
    ...
    function _calculatePayoutAmount(
        ...
        } else if (LifeCategory == "L") {
            payoutAmount = (PAYOUT_FACTOR_LARGE * sumInsured) / 100;
        } else {
            // small Lifes considered below deductible, no payout
            payoutAmount = 0;
        }
    }
}
